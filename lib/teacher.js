(function (root, factory) {
      ...
      return fetchFn(url, {
        ...
        return res.text().then(function (raw) {
          ...
          if (!res.ok) {
            ...
            else if (json.error && json.error.message) detail = json.error.message;
            else if (typeof json.message === 'string') detail = json.message;
            else detail = json.type || '';
            return {
              ok: false,
              practice: true,
              text: '',
  ...
  function practiceReply(userText, quest) {
    var q = String(userText || '');
    var teach = (quest && quest.teach) || 'We learn one small idea, then try it.';
    var tryLine = 'Look at the cards or type your try.';
    if (/tv|watch|screen/i.test(q)) {
      return 'TV stays locked until all 5 rooms have a stamp. We can come back to that later. ' + teach + ' ' + tryLine;
    }
    if (/joke/i.test(q)) {
      return 'A tiny joke: what do you call 2 + 3 when they hold hands? 5! Now back to our one idea. ' + teach + ' ' + tryLine;
    }
    if (/1000|million|billion/i.test(q)) {
      return 'That is a huge number! Today we use small numbers we can count on our fingers. ' + teach + ' ' + tryLine;
    }
    if (/why|what does|mean|how/i.test(q)) {
      return 'Good question. Here it is in simpler words. ' + teach + ' Now you try, then tell me what you notice.';
    }
    return 'I heard you. ' + teach + ' Let’s stay with this room and this one idea. ' + tryLine;
  }
